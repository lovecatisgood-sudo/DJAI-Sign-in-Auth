...
describe('minimal DJAI OIDC provider', () => {
  ...
  it('renders malformed authorization requests without exposing protocol details', async () => {
    const fixture = await providerFixture()

    const response = await fetch(`${fixture.issuer}/oauth/authorize?client_id=unknown-client`, {
      redirect: 'manual',
    })
    ...
    expect(response.headers.get('x-djai-error-reference')).toEqual(expect.stringMatching(/^[0-9a-f-]{36}$/))
    expect(body).toContain('Unable to continue')
  ...
  it('completes authorization when a fresh login switches the remembered provider account', async () => {
    const fixture = await providerFixture(false, client, [subject, secondSubject], true)
    const firstVerifier = randomBytes(48).toString('base64url')
    const first = authorizationUrl(fixture.issuer, 'first-account', firstVerifier)
    const firstResult = await followAuthorization(first, fixture)
    const firstCode = new URL(firstResult.url).searchParams.get('code')
    expect(firstCode).toBeTruthy()
    const firstToken = await fetch(`${fixture.issuer}/oauth/token`, {
      method: 'POST',
      headers: {
        authorization: `Basic ${Buffer.from(`${client.client_id}:${client.client_secret}`).toString('base64')}`,
        'content-type': 'application/x-www-form-urlencoded',
      },
      body: new URLSearchParams({ grant_type: 'authorization_code', code: firstCode!, redirect_uri: callbackUri, code_verifier: firstVerifier }),
    })
    expect(firstToken.status).toBe(200)

    const rememberedSecondGrant = new fixture.provider.Grant({ accountId: secondSubject, clientId: client.client_id })
    rememberedSecondGrant.addOIDCScope('openid email')
    const rememberedSecondGrantId = await rememberedSecondGrant.save()
    fixture.grants.set(`${secondSubject}:${client.client_id}`, rememberedSecondGrantId)

    const second = authorizationUrl(fixture.issuer, 'second-account')
    second.searchParams.set('prompt', 'login')
    const secondResult = await followAuthorization(second, fixture, firstResult.cookies)

    const callback = new URL(secondResult.url)
    expect(fixture.prompts.some((prompt) => prompt.startsWith('login:') && prompt.includes('login_prompt'))).toBe(true)
    expect(callback.origin + callback.pathname).toBe(callbackUri)
    expect(callback.searchParams.get('state')).toBe('second-account')
...
async function providerFixture(
  dynamicClient = false,
  registeredClient: ClientMetadata = client,
  loginSubjects: string[] = [subject],
  preparseProviderForms = false,
) {
  ...
  const identity = {
    bySubject: async (requested: string) => {
      if (requested === subject) return { subject, email: 'verified@example.com', emailVerified: true as const }
      if (requested === secondSubject) return { subject: secondSubject, email: 'second@example.com', emailVerified: true as const }
      return undefined
    },
  } as unknown as IdentityDirectory
  const grants = new Map<string, string>()
  const confirmations = { grantId: async (accountId: string, clientId: string) => grants.get(`${accountId}:${clientId}`) } as unknown as ConfirmationRepository
  ...
  const dispatch = (request: IncomingMessage, response: ServerResponse): void => {
    const loginSubject = loginSubjects[Math.min(loginIndex, loginSubjects.length - 1)] ?? subject
    void handleRequest(provider, request, response, prompts, grants, loginSubject, () => { loginIndex += 1 })
  }
  if (preparseProviderForms) {
    const app = express()
    app.use(express.urlencoded({ extended: false, limit: '16kb', parameterLimit: 20 }))
    app.use((request, response) => dispatch(request, response))
    server.on('request', app)
  } else {
    server.on('request', dispatch)
  }
  return { issuer, provider, prompts, grants }
}
  ...
  try {
    if (request.url?.startsWith('/interaction/')) {
      ...
      if (details.prompt.name === 'login') {
        consumeLogin()
        await provider.interactionFinished(request, response, { login: { accountId: loginSubject, amr: ['pwd'] } }, { mergeWithLastSubmission: false })
        return
      }
      if (details.prompt.name === 'consent' && details.session) {
        ...
        grants.set(`${details.session.accountId}:${String(details.params.client_id)}`, grantId)
...
function authorizationUrl(issuer: string, state: string, verifier = randomBytes(48).toString('base64url')): URL {
  const challenge = createHash('sha256').update(verifier).digest('base64url')
  const authorize = new URL('/oauth/authorize', issuer)
  authorize.search = new URLSearchParams({
    response_type: 'code', client_id: client.client_id, redirect_uri: callbackUri,
    scope: 'openid email', state, nonce: `${state}-nonce`,
    code_challenge: challenge, code_challenge_method: 'S256',
  }).toString()
  return authorize
}
...
async function followAuthorization(
  ...
  for (let index = 0; index < 24; index += 1) {
    ...
    if (location) {
      next = new URL(location, next).toString()
      if (next.startsWith(callbackUri)) return { url: next, cookies }
      continue
    }
    ...
    if (!action || fields.length === 0) throw new Error(`Authorization stopped at ${next} with HTTP ${response.status}`)
    const form = new URLSearchParams(fields.map((match): [string, string] => [match[1]!, match[2]!]))
    const formUrl = new URL(action, fixture.issuer).toString()
    const formResponse = await fetch(formUrl, {
      method: 'POST', redirect: 'manual',
      headers: { ...(cookies ? { cookie: cookies } : {}), 'content-type': 'application/x-www-form-urlencoded' },
      body: form,
    })
    cookies = mergeCookies(cookies, formResponse.headers.getSetCookie())
    const formLocation = formResponse.headers.get('location')
    if (!formLocation) throw new Error(`Authorization form stopped at ${formUrl} with HTTP ${formResponse.status}`)
    next = new URL(formLocation, formUrl).toString()
  }
  throw new Error('Authorization did not reach the client callback')
}
